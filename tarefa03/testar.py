...
# FIXME: criar verificador da solução
class Task(BaseTask):
    def compare_files(self, out, res):
        in_ = self.read_file_utf8(out.replace(".out", ".in")).split()
        out = self.read_file_utf8(out).split()
        res = self.read_file_utf8(res).split()
        m, n = map(int, in_)
        assert out, "saída em branco"
        r_out = out.pop(0)
        r_res = res.pop(0)
        assert r_out == r_res, f"saída {r_out}, deveria ser {r_res}"
        assert len(out) == len(res), "formato inválido"
        r_out = int(r_out)
        if r_out == -1:
            return True
        assert all(c in [".", "p", "r"] for c in out), "peça inválida"
        assert sum(1 for c in out if c == "r") == m, f"número de rainhas deve ser {m}"
        assert sum(1 for c in out if c == "p") == r_out, f"número de peões deve ser {r_out}"

        def check_attack(i, j, di, dj):
            indices = [(i + k * di, j + k * dj) for k in range(n)]
            indices = [i * n + j for i, j in indices if 0 <= i < n and 0 <= j < n]
            line = "".join(out[i] for i in indices if out[i] != ".")
            assert "rr" not in line, "rainhas se atacam"

        for i in range(n):
            check_attack(0, i, 1, 0)
            check_attack(i, 0, 0, 1)
            check_attack(i, 0, -1, 1)
            check_attack(i, 0, 1, 1)
            check_attack(i, n - 1, -1, -1)
            check_attack(i, n - 1, 1, -1)

        return True

    def teste_1_rainhas(self):
        binary = self.compile(["rainhas.c"])
        self.test_cases(binary, self.case_range("enumera{}.in", 1, 4))

    def teste_2_rainhas(self):
        binary = self.compile(["rainhas.c"])
        self.test_cases(binary, self.case_range("enumera{}.in", 5, 5))

    def teste_3_rainhas(self):
        binary = self.compile(["rainhas.c"])
        self.test_cases(binary, self.case_range("rainhas{}.in", 1, 10))
